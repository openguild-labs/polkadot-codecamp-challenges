  ...
  persist(
      ...
      return {
        ...
        restoreAgent: async () => {
          ...
          try {
            ...
            const { PolkadotAgentKit } =
              await import("@polkadot-agent-kit/sdk");

            // Use mnemonic if available, otherwise use privateKey
            ...
            if (config.mnemonic) {
              agentConfig.mnemonic = config.mnemonic;
            } else if (config.privateKey) {
              agentConfig.privateKey = config.privateKey;
            }

            const agentKit = new PolkadotAgentKit(agentConfig);
        ...
        initializeAgent: async () => {
          ...
          try {
            ...
            console.log("[AgentStore] Agent initialized with config:", {
              keyType: config.keyType,
              chains: config.chains,
              usesMnemonic: !!config.mnemonic,
            });
            ...
            try {
              const address =
                agentKit.getAddress?.() ||
                agentKit.address ||
                "Address method not available";
              console.log("[AgentStore] Derived wallet address:", address);
            } catch (e) {
              console.log("[AgentStore] Could not get address:", e);
            }
    ...
    },
    {
      ...
      onRehydrateStorage: () => (state?: AgentState) => {
        ...
        if (state) {
          console.log(
            "[AgentStore] Rehydration: Clearing agentKit to force restoration",
          );
          state.agentKit = null;
...
export const useAgentRestore = () => {
  const { isInitialized, agentKit, config, sessionExpiry, restoreAgent } =
    useAgentStore();
  ...
  React.useEffect(() => {
    ...
    if (
      hasAttemptedRestore ||
      (agentKit && typeof agentKit === "object" && agentKit.getActions)
    ) {
      return;
    }
    ...
    const timer = setTimeout(() => {
      ...
      if (sessionExpiry) {
        ...
        if (now > sessionExpiry) {
          console.log(
            "[useAgentRestore] Session expired, user needs to reconnect",
          );
          setHasAttemptedRestore(true);
          return;
        }
        ...
        console.log(
          "[useAgentRestore] No session expiry, user needs to initialize",
        );
      }
