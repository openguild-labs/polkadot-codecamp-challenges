...
export default function ChatWindow({ messages, agent }: ChatWindowProps) {
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-4 flex flex-col bg-white">
      {messages.length === 0 ? (
        ...
        messages.map((message, index) => (
          <div key={message.id} className={`flex ${message.type === "user" ? "justify-end" : "justify-start"}`}>
            <div
              ...
              style={{
                background: message.type === "user" ? "#ff6b9d" : "#fef3c7",
            ...
            >
              ...
              <p className="text-xs mt-2 opacity-70 mt-3">{message.timestamp}</p>
            </div>
          </div>
        ))
      )}
